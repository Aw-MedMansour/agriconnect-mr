  ...
  return (
    <header className="sticky top-0 z-40 w-full border-b border-slate-200 bg-white/95 shadow-sm backdrop-blur-xl">
      <div className="mx-auto max-w-7xl px-3 sm:px-5">
        <div className="flex min-h-[78px] items-center justify-center py-2 sm:min-h-[92px]">
          <button onClick={goHome} className="group flex min-w-0 items-center justify-center gap-2.5 text-center sm:gap-3" aria-label="AgriConnect">
            <img src={agriLogo} alt="Logo AgriConnect" className="h-14 w-14 shrink-0 rounded-2xl object-cover shadow-sm ring-1 ring-slate-200 sm:h-16 sm:w-16" />
            <span className="min-w-0 text-start">
              <span className="flex items-center gap-2">
                <span className="truncate text-xl font-black tracking-tight text-[#0a66c2] sm:text-3xl">AgriConnect</span>
                <span className="rounded-md bg-[#0a66c2] px-1.5 py-0.5 text-[8px] font-black tracking-wider text-white sm:text-[10px]">PRO</span>
              </span>
              <span className="mt-0.5 block truncate text-[10px] font-semibold text-slate-500 sm:text-xs">{t('Le réseau professionnel agricole')}</span>
            </span>
          </button>
        </div>
        ...
        <div className="relative flex min-h-[58px] items-center justify-between gap-2 border-t border-slate-100 py-2">
          ...
          <div className="flex min-w-0 items-center justify-end gap-1 sm:gap-2">
            <div ref={profileRef} className="relative">
              {currentUser ? (
                <button onClick={() => setProfileOpen(v => !v)} className="flex h-10 max-w-[150px] items-center gap-2 rounded-full border border-slate-200 bg-white px-1.5 pe-2.5 text-start hover:bg-slate-50" aria-expanded={profileOpen}>
                  <Avatar src={currentUser.avatar} name={currentUser.name} seed={currentUser.id} className="h-7 w-7 shrink-0" textClassName="text-[9px]" />
            ...
            <div className="relative">
              <button onClick={() => setSearchOpen(v => !v)} className={`flex h-10 w-10 items-center justify-center rounded-full border transition ${searchOpen ? 'border-[#0a66c2] bg-blue-50 text-[#0a66c2]' : 'border-slate-200 text-slate-600 hover:bg-slate-50'}`} aria-label={t('Rechercher')}><Search className="h-4 w-4" /></button>
      ...
      <nav aria-label="Navigation principale" className="border-t border-slate-100 bg-slate-50/80">
        <div className="mx-auto flex max-w-7xl items-stretch gap-1 overflow-x-auto px-3 no-scrollbar sm:justify-center sm:px-5">
          {modules.map(({ id, label, icon: Icon }) => {
            ...
            return <button key={id} onClick={() => setActiveModule(id)} className={`relative flex shrink-0 items-center gap-1.5 px-3 py-3 text-[11px] font-bold transition sm:px-4 sm:text-xs ${active ? 'text-[#0a66c2]' : 'text-slate-600 hover:text-slate-900'}`}><Icon className="h-3.5 w-3.5" />{t(label)}{active && <span className="absolute inset-x-2 bottom-0 h-0.5 rounded-full bg-[#0a66c2]" />}</button>;
          })}
        </div>
      </nav>
    </header>
