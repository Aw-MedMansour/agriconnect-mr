...
export default function PlantAIChat({ currentUser, onRequireAuth, fullScreen = false }) {
  ...
  return (
    <section className={`flex min-h-0 flex-col overflow-hidden bg-white ${fullScreen ? 'h-full w-full rounded-none border-0 shadow-none' : 'h-[min(72vh,700px)] min-h-[500px] rounded-3xl border border-slate-200 shadow-lg'}`}>
      ...
      <form onSubmit={event => { event.preventDefault(); send(); }} className="shrink-0 border-t border-slate-200 bg-white p-2.5 pb-[max(0.625rem,env(safe-area-inset-bottom))] sm:p-3">
        {!currentUser ? <button type="button" onClick={onRequireAuth} className="flex w-full items-center justify-center gap-2 rounded-full bg-amber-500 py-3 text-xs font-bold text-white hover:bg-amber-600"><Lock className="h-4 w-4" />{t('Connectez-vous pour discuter avec Plant AI')}</button> : <div className="flex items-end gap-1.5 rounded-xl border border-slate-300 bg-slate-50 p-1 focus-within:border-[#0a66c2] focus-within:ring-2 focus-within:ring-blue-100"><textarea ref={inputRef} rows={1} value={input} onChange={event => setInput(event.target.value)} onFocus={event => event.currentTarget.scrollIntoView({ block: 'nearest' })} onKeyDown={event => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); send(); } }} placeholder={t('Posez votre question agricole…')} className="max-h-32 min-h-10 min-w-0 flex-1 resize-none overflow-y-auto bg-transparent px-2.5 py-2 text-base font-normal leading-6 outline-none sm:text-sm sm:leading-5" /><button type="submit" disabled={isThinking || !input.trim()} aria-label={t('Envoyer le message à Plant AI')} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-[#0a66c2] text-white disabled:opacity-40"><Send className="h-4 w-4" /></button></div>}
      </form>
    </section>
  );
}